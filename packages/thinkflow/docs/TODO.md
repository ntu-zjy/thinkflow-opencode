...
- [ ] **伙伴 Agent（记忆管理伙伴）**
  并行于画布功能的专属 Agent；持久单会话（上下文达阈值时自动 compact）；与记忆库双向绑定（提示词驱动 Agent 自主读写记忆分类）；计费按时间或额度；首次使用时引导用户建立人设/风格记忆。

- [ ] **让记忆机制和画布能够更加无缝的衔接**
  AgentNode 新增「自动记忆」开关；开启后运行完成自动提取关键信息写入记忆节点，用户可选择是否保存；支持直接从输出作品存入记忆。

- [ ] **增加输出节点的批量输出模式（抽卡）**
  单个输出节点可配置重复生成次数，多次结果并排展示，用户选择最佳版本。
