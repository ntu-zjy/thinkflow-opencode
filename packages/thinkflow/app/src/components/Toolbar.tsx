...
export function Toolbar({ theme, onToggleTheme, onRestartTour }: ToolbarProps) {
  ...
  const creditsInsufficient = useCanvasStore((s) => s._creditsInsufficient)
  const clearCreditsInsufficient = useCanvasStore((s) => s.clearCreditsInsufficient)
  ...
  // 积分不足时刷新积分显示
  useEffect(() => {
    if (creditsInsufficient) refreshCredits()
  }, [creditsInsufficient]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <>
      <div className="tf-toolbar">
        ...
        {/* 积分显示（已登录时） */}
        {user && (
          <div className="tf-toolbar__credits" title={`每日积分 + 永久积分`}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" style={{ opacity: 0.7 }}>
              <circle cx="12" cy="12" r="10"/>
              <path fill="var(--accent-text)" d="M12 6v6l4 2"/>
            </svg>
      ...
      {/* ── 积分不足弹窗 ── */}
      {creditsInsufficient && (
        <div className="tf-credits-modal-overlay" onClick={clearCreditsInsufficient}>
          <div className="tf-credits-modal" onClick={(e) => e.stopPropagation()}>
            <div className="tf-credits-modal__icon">💎</div>
            <h3 className="tf-credits-modal__title">积分不足</h3>
            <p className="tf-credits-modal__desc">
              今日免费积分已用完。充值积分包可继续创作，或明天积分自动刷新（每日赠送 30 积分）。
            </p>
            <div className="tf-credits-modal__actions">
              <a href="/pricing" className="tf-credits-modal__buy">
                去充值 →
              </a>
              <button className="tf-credits-modal__cancel" onClick={clearCreditsInsufficient}>
                明天再来
              </button>
            </div>
          </div>
