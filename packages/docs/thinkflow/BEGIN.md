...
1. 信息流：信息流是一种信息来源，比如用户设置信息流为GitHub的每日热榜，那么用户点击运行的时候，这条输入就会是运行时刻爬取的GitHub的每日热榜。
...
## 想法
想法其实就是在上面输入的基础上，用户当下的灵感，或者一个指令
## 输出
输出=（输入+想法）-> Agent
输出需要适配多个内容创作平台，用户可以预先配置内容创作平台的格式。
可以链接出来多个输出，可以设置重复运行次数。
输出类型比如小红书图文，长文本，小说，知乎，公众号等。
也可以输出记忆。
## 记忆
...
## Agent+plugin
### 核心能力
- **基础 Agent**：基于 OpenCode 的 Agent 运行内核
- **Plugin 扩展**：通过插件定制 Agent 能力
* Opencode Plugin参考https://opencode.ai/docs/plugins/
...
- **画布引擎**：ReactFlow 
- **UI 组件**：Ant Design，Chakra UI（禁止采用表情包作为组件设计）

## 后端
- **Agent 内核**：OpenCode
- **Plugin 系统**：OpenCode Plugin SDK
- **数据库**：SQLite (本地) + 可选云端同步
- **文件存储**：本地文件系统 + 可选云存储

## AI 能力
- **默认模型**：Kimi-k2.6(Agent模型) (通过 OpenCode 的openrouter 配置)
- **图像理解**：Kimi-k2.6支持多模态模型
...
3. **添加想法**：双击画布空白处快速输入
4. **配置 Agent**：点击 Agent 节点选择插件组合
5. **运行**：点击节点「▶ 运行」或全局「运行全部」
6. **查看输出**：点击输出节点预览 → 复制/导出
7. **保存记忆**：一键将本次输出存为记忆
...
# 支付方案
暂时采用ZPAY绑定支付宝的支付模式
管理员也可以给特定的用户增加用量（如果用户没有通过平台方式进行交易，而是和管理员直接转账购买）
